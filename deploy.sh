...
# CRITICAL: Copy only configs, NOT data volumes
# If we copy mysql-data, the new password in .env will be ignored by MySQL!
if [ -d "$SRC_DIR/.configs" ]; then
    echo "⚙️ Syncing configuration files..."
    mkdir -p "$TARGET_DIR/.configs"
    # Copy only files (not directories like mysql-data)
